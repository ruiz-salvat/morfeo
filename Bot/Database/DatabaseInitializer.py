...
from Util.Constants import symbols_table_name, prices_table_name, instances_table_name, instance_states_table_name, \
    trades_table_name, customers_table_name, patterns_table_name, database_name, test_database_name
...
class DatabaseInitializer:
    ...
    def initialize_database(self):
        if database_name not in self.db_connector.db_client.list_database_names():
            ...
            trades.append(Trades(0, time.time() - 10000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 9500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 9000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 8500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 8000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 7500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 7000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 6500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 6000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 5500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 5000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 4500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 4000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 3500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 3000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 2500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 2000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 1500, 'SELL', 430, 11, 1))
            trades.append(Trades(0, time.time() - 1000, 'BUY', 420, 10, None))
            trades.append(Trades(0, time.time() - 500, 'SELL', 430, 11, 1))
