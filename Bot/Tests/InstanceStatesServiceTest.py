...
from Util.Constants import insert_instance_states_db_msg, instance_states_table_name, instances_pk, \
    ...
    delete_instance_states_db_msg, delete_instance_states_db_error_msg, get_instance_states_error_msg
...
def InstanceStatesService_GetElement_Equal():
    ...
    element = service.get_element(valid_id)

    assert element['budget'] == test_budget, 'the retrieved budget should be as it is expected'
...
def InstanceStatesService_GetElement_Error():
    ...
    assert element == get_instance_states_error_msg, 'the error message should be returned'
